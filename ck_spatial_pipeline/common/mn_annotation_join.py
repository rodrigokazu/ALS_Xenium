#!/usr/bin/env python3
# ========================================================================================
# ALS_Xenium repo commentary | ck_spatial_pipeline/common/mn_annotation_join.py
#
...
def join_is_MN(adata, label, run_dir=None, verbose=True,
    ...
    Always sets:
      ...
      uns['is_MN_join_key']       the winning candidate key name ('n/a (no confirmed
                                  join)' when there is none)
      ...
      uns['is_MN_final_ncells']   authoritative _final per-sample cell count (-1 when
                                  unavailable -- a real count is always > 0)
      uns['is_MN_rowcount_ratio'] source_nrows / _final cell count (NaN when not computed)

    NOTE (2026-08-03): these three MUST NOT be written as Python None. anndata has no
    registered reader for IOSpec(encoding_type='null'), so a None in .uns writes fine but
    makes the h5ad UNREADABLE on load (IORegistryError on the offending key) -- in EVERY
    anndata version incl. 0.11.4, since the None write-side registration is commented out
    upstream. Use the sentinels above instead. Same rule applies to uns['spatial_offset']
    in VH_concatenate_FINAL.py, which uses {} for the empty case.
    ...
    adata.uns["is_MN_join_key"] = "n/a (no confirmed join)"
    adata.uns["mn_columns_added"] = []
    adata.uns["is_MN_provenance"] = "n/a (no confirmed join)"
    adata.uns["is_MN_source_nrows"] = 0
    adata.uns["is_MN_final_ncells"] = -1
    adata.uns["is_MN_rowcount_ratio"] = float("nan")
    ...
    adata.uns["is_MN_final_ncells"] = int(final_count) if final_count is not None else -1
