# ========================================================================================
# ALS_Xenium repo commentary | ck_spatial_pipeline/1_novae/novae_INDEPENDENT_FINAL.py
#
...
STATUS_KEY = "status"
MN_KEY = "is_MN"
# Legend/label text for whatever MN_KEY points at. Defaults keep every _FINAL run identical.
# The AUG5 driver repoints MN_KEY to 'is_neuron' (that build has no MN call at all) and sets
# this to 'neuron' so no figure claims a motor-neuron label it does not have.
MN_LABEL = "MN"
...
def spatial_plot(ad, dom_key, sample_name, n_dom, path):
    ...
    if mmn.any():
        ax.scatter(xy[mmn, 0], xy[mmn, 1], s=POINT_SIZE * 6, facecolors="none",
                   edgecolors="k", linewidths=0.4, label=f"{MN_LABEL} (n={int(mmn.sum())})", zorder=5)
    ax.set_aspect("equal"); ax.invert_yaxis()
    ax.set_title(f"{sample_name} — INDEPENDENT niches (~{n_dom} domains) + {MN_LABEL}s")
...
def process_one(task_id):
    ...
    for tgt, key in dkeys.items():
        ...
        rows.append({
            ...
            "mark_column": MN_KEY, "mark_label": MN_LABEL,
