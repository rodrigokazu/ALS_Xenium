#!/usr/bin/env python
# ========================================================================================
# ALS_Xenium repo commentary | ck_spatial_pipeline/side_imaging_overlays/hero_crop_FINAL.py
#
...
def _render(sample, tag, cx, cy, half, full, s18_full, Wpx, Hpx, cells, cb_all, palette,
    ...
    h.append(Line2D([0],[0], marker='o', linestyle='none', markersize=8, markerfacecolor='none',
                    markeredgecolor=xo.MN_EDGE, markeredgewidth=1.6, label=xo.MARK_LABEL))
    if txd:
    ...
    ax.set_title(f"{sample}  |  {xo.MARK_LABEL}s in the ventral horn (Xenium; DAPI + 18S){mode}",
                 color="white", fontsize=11)
    for e in ("png", "pdf"):
        fig.savefig(f"{outdir}/{sample}__hero_{xo.MARK_ABBR}_{tag}.{e}", dpi=450,
                    bbox_inches="tight", facecolor="black")
    plt.close(fig)
    print(f"[{sample}] wrote hero_{xo.MARK_ABBR}_{tag}  ({ncells} cells, {nmn} {xo.MARK_ABBR}) "
          f"@ {cx:.0f},{cy:.0f}", flush=True)
...
def main(sample, bundle, h5, outdir, half=115.0, center=None, topk=4, genes=None, min_sep=450.0):
    ...
    obs = xo.read_obs(h5, [dkey, xo.MARK_COL])
    dom = obs[dkey]
    mn = xo._as_bool(obs[xo.MARK_COL]) if xo.MARK_COL in obs else pd.Series(False, index=dom.index)
    cells = xo.load_cells(bundle, dom, mn)
    mn_xy = cells.loc[cells["is_MN"], ["x_centroid", "y_centroid"]].to_numpy()
    print(f"[{sample}] {xo.MARK_ABBR} cells {len(mn_xy)}", flush=True)
    # ---- mark-column GATE ----
    if len(mn_xy) == 0:
        print(f"[{sample}] SKIP heroes: {xo.MARK_COL}.sum()==0 (absent or empty). "
              f"Domain overview/detail + transcript overlays are produced by "
              f"xenium_overlay_FINAL; heroes require {xo.MARK_COL}.", flush=True)
        return
