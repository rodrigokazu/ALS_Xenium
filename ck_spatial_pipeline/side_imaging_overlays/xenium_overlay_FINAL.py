#!/usr/bin/env python
# ========================================================================================
# ALS_Xenium repo commentary | ck_spatial_pipeline/side_imaging_overlays/xenium_overlay_FINAL.py
#
...
# Which obs column supplies the cyan-outlined cells, and what to call them in the figures.
# The defaults reproduce the previous behaviour exactly, so every existing caller is unchanged.
# The AUG5 build carries no is_MN at all (Marcel's neuron call is the only real annotation on
# that universe), so the AUG5 report driver sets these to 'is_neuron' / 'neuron' and the same
# machinery outlines neurons. Set them on the MODULE before calling main().
MARK_COL   = "is_MN"
MARK_LABEL = "motor neuron"
MARK_ABBR  = "MN"
...
def domain_legend(ax, present, palette, with_mn=True, tx_genes=None, swatch=11, fontsize=10):
    ...
    if with_mn:
        h.append(Line2D([0],[0], marker='o', linestyle='none', markersize=swatch, markerfacecolor='none',
                        markeredgecolor=MN_EDGE, markeredgewidth=1.6, label=MARK_LABEL))
...
def main(sample, bundle, h5, outdir, crop_center=None, genes=None):
    ...
    obs = read_obs(h5, [dkey, MARK_COL])
    dom = obs[dkey]
    mn = _as_bool(obs[MARK_COL]) if MARK_COL in obs else pd.Series(False, index=dom.index)
    n_mn = int(mn.sum())
    print(f"[{sample}] domains {dom.value_counts().to_dict()}  {MARK_ABBR} {n_mn}"
          f"{f'  ({MARK_COL} absent/empty -> {MARK_ABBR} marks suppressed)' if n_mn == 0 else ''}",
          flush=True)
    ...
    if polys_ok:
        print(f"[{sample}] polygons {len(polys)}  {MARK_ABBR} polys {int(isMN.sum())}", flush=True)
        dom_set = set(doms)
    else:
    ...
    ax.set_title(f"{sample}  |  ventral-horn detail  (DAPI + 18S; {n_in:,} cells, "
                 f"{n_mn_in} {MARK_ABBR})", color="white", fontsize=10)
    domain_legend(ax, present, palette, with_mn=len(mn_xy_all) > 0, tx_genes=genes)
