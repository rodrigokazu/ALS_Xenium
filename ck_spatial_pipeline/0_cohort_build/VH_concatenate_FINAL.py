#!/usr/bin/env python3
# ========================================================================================
# ALS_Xenium repo commentary | ck_spatial_pipeline/0_cohort_build/VH_concatenate_FINAL.py
#
...
Outputs (see final_config):
  per-sample : H5AD_DIR/<LABEL>__MNcorrected_FINAL.h5ad   (single-slide: spatial==
               spatial_orig == true microns; uns['spatial_offset']={})
...
def main():
    ...
    for i_dir, (label, sample_dir) in enumerate(kept_list, start=1):
        ...
        if "spatial" in adata.obsm:
            ...
            # {} not None: a None in .uns writes but makes the h5ad unreadable
            # (no anndata reader for encoding_type='null'). {} matches the
            # dict(uns.get("spatial_offset", {})) consumer above.
        ...
        if "spatial_orig" in aw.obsm:
            ...
            aw.uns["spatial_offset"] = {}   # not None -- see note above
