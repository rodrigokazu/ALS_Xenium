...
The generation before `ck_spatial_pipeline`, built on Marcel's Gaussian re-segmentation
(`Ranger_procd_mw_gausss`). Only the parts with no successor in `ck_spatial_pipeline` are here.
...
`ck_spatial_pipeline` replaces it yet; point it at the `_FINAL` niche object when porting.
...
`ck_spatial_pipeline/common/ps_io_FINAL_fix.py`, so running these scripts against `_FINAL` output needs
that fix first.
...
A copy of the shared plotting helpers, byte-identical to the one in `ck_spatial_pipeline`. It lives here because every script in this folder imports it by bare name, resolving against its own directory. Change one, change both.
