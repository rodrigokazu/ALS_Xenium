# ========================================================================================
# ALS_Xenium repo commentary | archive/novae_v1/novae_persample_niches.py
#
# Where the Novae work on this cohort started. Retained for provenance. Superseded twice over:
# first by the MN-corrected version on Marcel's Gaussian re-segmentation, then by
# ck_spatial_pipeline/1_novae/novae_JOINT_FINAL.py on the _final segmentation.
#
# It fitted one joint model across all 20 slides and produced the six-niche solution that
# first showed the cord anatomy coming out of the data: a white-matter and meningeal rim, a
# grey-matter core, and a central canal domain, with only 1,413 cells left unassigned out of
# 1.68 million. That result is what justified everything built on Novae afterwards.
#
# Do not run this against current data. It points at the old Ranger_procd object, its domain
# ids do not correspond to any later run, and it predates the num_workers fix that stopped the
